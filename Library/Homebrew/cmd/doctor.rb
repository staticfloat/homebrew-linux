...
def check_for_git
  unless system "#{SystemCommand.which_s} git > /dev/null" then <<-EOS.undent
...
def check_git_newline_settings
  return unless system "#{SystemCommand.which} git"
...
def check_git_status
  return unless system "#{SystemCommand.which_s} git > /dev/null"
...
def check_for_enthought_python
  if system "#{SystemCommand.which_s} enpkg > /dev/null" then <<-EOS.undent
...
def check_for_bad_python_symlink
  return unless system "#{SystemCommand.which_s} > /dev/null python"
